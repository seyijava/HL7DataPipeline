...
class HapiParser(validate: Boolean, explodeFieldComponents: Boolean = true, replaceNames: Boolean = false) extends Serializable
{
  
  

  @transient lazy val   context = new DefaultHapiContext()
  @transient lazy val parser = context.getGenericParser()
